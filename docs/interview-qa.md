...
- **[四、分层摘要、Embedding 混合检索与 Checkpoint](#四分层摘要embedding-混合检索与-checkpoint)**
  - [Q31：为什么不能把完整 VideoContext 直接交给模型？](#q31为什么不能把完整-videocontext-直接交给模型)
  - [Q32：五分钟分块和关键词、Embedding 混合检索的完整流程是什么？](#q32五分钟分块和关键词embedding-混合检索的完整流程是什么)
  - [Q33：为什么同时需要关键词和 Embedding？各自权重是多少？](#q33为什么同时需要关键词和-embedding各自权重是多少)
  - [Q34：Qdrant 或 Embedding 服务不可用怎么办？](#q34qdrant-或-embedding-服务不可用怎么办)
  - [Q35：为什么先检索再 Planner，会不会因为第一次检索不完整导致计划遗漏？](#q35为什么先检索再-planner会不会因为第一次检索不完整导致计划遗漏)
  - [Q36：Planner 和 Critic 分别怎样使用检索？](#q36planner-和-critic-分别怎样使用检索)
  - [Q37：摘要漏信息、TopK 没命中或向量服务失败怎么办？](#q37摘要漏信息topk-没命中或向量服务失败怎么办)
  - [Q38：Checkpoint 与 MQ 重试、幂等和缓存有什么区别？](#q38checkpoint-与-mq-重试幂等和缓存有什么区别)
  - [Q39：Checkpoint 保存什么？为什么要区分视频级和目标级？](#q39checkpoint-保存什么为什么要区分视频级和目标级)
  - [Q40：Checkpoint 怎样持久化？Redis 挂了怎么办？](#q40checkpoint-怎样持久化redis-挂了怎么办)
  - [Q41：不同阶段失败后怎样恢复？](#q41不同阶段失败后怎样恢复)
  - [Q42：如何避免假完成、并发恢复和版本污染？](#q42如何避免假完成并发恢复和版本污染)
  - [Q43：如何证明检索和 Checkpoint 确实有价值？](#q43如何证明检索和-checkpoint-确实有价值)
- **[五、分片上传与断点续传](#五分片上传与断点续传)**
  - [Q44：为什么普通 Multipart 上传不适合长视频？当前完整流程是什么？](#q44为什么普通-multipart-上传不适合长视频当前完整流程是什么)
  - [Q45：为什么使用 uploadId 管上传，MD5 在哪里发挥作用？](#q45为什么使用-uploadid-管上传md5-在哪里发挥作用)
  - [Q46：为什么坚持「先写 MinIO，再记录 Redis」？](#q46为什么坚持先写-minio再记录-redis)
  - [Q47：合并过程怎样保证顺序和幂等？](#q47合并过程怎样保证顺序和幂等)
  - [Q48：为什么分片限制在 5MB、总分片限制为 410？](#q48为什么分片限制在-5mb总分片限制为-410)
  - [Q49：上传到 99% 断网、Redis 故障或合并中宕机会怎样？](#q49上传到-99-断网redis-故障或合并中宕机会怎样)
  - [Q50：怎样防止用户访问别人的上传任务？](#q50怎样防止用户访问别人的上传任务)
  - [Q51：如何证明断点续传真的有效？](#q51如何证明断点续传真的有效)
- **[六、RocketMQ 异步调度与 Redisson 幂等](#六rocketmq-异步调度与-redisson-幂等)**
  - [Q52：为什么需要 RocketMQ，而不是本地线程池、Kafka 或 RabbitMQ？](#q52为什么需要-rocketmq而不是本地线程池kafka-或-rabbitmq)
  - [Q53：从提交分析到消费者完成，完整流程是什么？](#q53从提交分析到消费者完成完整流程是什么)
  - [Q54：任务长时间没响应，或者用户手动点了重新执行，分别怎么处理？](#q54任务长时间没响应或者用户手动点了重新执行分别怎么处理)
  - [Q55：为什么幂等 Key 不能只有视频 MD5？activeKey、锁、completedKey、Checkpoint 各自解决什么？](#q55为什么幂等-key-不能只有视频-md5activekey锁completedkeycheckpoint-各自解决什么)
  - [Q56：为什么使用 Redisson，而不是 JVM 锁或手写 SETNX？WatchDog 怎样工作？](#q56为什么使用-redisson而不是-jvm-锁或手写-setnxwatchdog-怎样工作)
  - [Q57：重复消费、执行失败三次和失败消息分别怎么处理？](#q57重复消费执行失败三次和失败消息分别怎么处理)
  - [Q58：毒消息（结构非法的消息）怎么处理？](#q58毒消息结构非法的消息怎么处理)
  - [Q59：SSE 断开、消息积压或视频被删除时，业务状态怎么闭环？](#q59sse-断开消息积压或视频被删除时业务状态怎么闭环)
  - [Q60：如何证明异步和幂等设计有效？](#q60如何证明异步和幂等设计有效)
- **[七、Redis 令牌桶限流与指数退避重试](#七redis-令牌桶限流与指数退避重试)**
  - [Q61：为什么视频 Agent 必须限流，当前令牌桶怎样实现？](#q61为什么视频-agent-必须限流当前令牌桶怎样实现)
  - [Q62：为什么选择令牌桶，而不是固定窗口或漏桶？](#q62为什么选择令牌桶而不是固定窗口或漏桶)
  - [Q63：用户许可拿到了，但全局许可失败或 MQ 投递失败怎么办？](#q63用户许可拿到了但全局许可失败或-mq-投递失败怎么办)
  - [Q64：Redis 不可用时选择放行还是拒绝？](#q64redis-不可用时选择放行还是拒绝)
  - [Q65：第三方 API 重试和 RocketMQ 重试有什么区别？](#q65第三方-api-重试和-rocketmq-重试有什么区别)
  - [Q66：哪些错误应该重试，哪些应该立即失败？](#q66哪些错误应该重试哪些应该立即失败)
  - [Q67：为什么使用指数退避，当前方案还有什么不足？](#q67为什么使用指数退避当前方案还有什么不足)
  - [Q68：重试会不会造成重复扣费和重复结果？](#q68重试会不会造成重复扣费和重复结果)
  - [Q69：怎样验证限流和重试不是摆设？](#q69怎样验证限流和重试不是摆设)
- **[八、底层技术地基](#八底层技术地基)**
  - [Q70：RocketMQ 消息是「至少一次」还是「恰好一次」？怎么应对重复？](#q70rocketmq-消息是至少一次还是恰好一次怎么应对重复)
  - [Q71：Redisson 分布式锁原理，什么是 WatchDog？](#q71redisson-分布式锁原理什么是-watchdog)
  - [Q72：SETNX 和 Redisson 锁的区别？](#q72setnx-和-redisson-锁的区别)
  - [Q73：CompletableFuture 和线程池怎么配合？为什么不直接用多线程？](#q73completablefuture-和线程池怎么配合为什么不直接用多线程)
  - [Q74：FFmpeg 的 select 过滤器、scene score、showinfo 是干什么的？](#q74ffmpeg-的-select-过滤器scene-scoreshowinfo-是干什么的)
  - [Q75：什么是感知哈希（dHash），为什么能去重？](#q75什么是感知哈希dhash为什么能去重)
  - [Q76：Embedding 和余弦相似度是什么？和关键词检索各擅长什么？](#q76embedding-和余弦相似度是什么和关键词检索各擅长什么)
  - [Q77：SSE 和 WebSocket、轮询的区别？为什么用 SSE？](#q77sse-和-websocket轮询的区别为什么用-sse)
  - [Q78：什么是 Flyway？项目里怎么用？](#q78什么是-flyway项目里怎么用)
  - [Q79：MyBatis-Plus 的 upsert 是干什么的？](#q79mybatis-plus-的-upsert-是干什么的)
- **[九、高频陷阱追问](#九高频陷阱追问)**
  - [Q80：你刚说「内容级去重」，是不是实现了文件秒传？](#q80你刚说内容级去重是不是实现了文件秒传)
  - [Q81：你说了半天幂等，分布式锁能保证「恰好一次」吗？](#q81你说了半天幂等分布式锁能保证恰好一次吗)
  - [Q82：Critic 和 Executor 用同一个模型，是不是自我开卷？](#q82critic-和-executor-用同一个模型是不是自我开卷)
  - [Q83：检索语义权重是 0.7 吗？](#q83检索语义权重是-07-吗)
  - [Q84：你说「并行 ASR + OCR」，是并行调两个大模型吗？](#q84你说并行-asr--ocr是并行调两个大模型吗)
  - [Q85：视频处理这么快，是不是把整个视频给模型了？](#q85视频处理这么快是不是把整个视频给模型了)
  - [Q86：Token 预算耗尽重试一下不就好了？](#q86token-预算耗尽重试一下不就好了)
  - [Q87：为什么不用现成的向量检索 + 大模型 RAG 框架（LangChain / LlamaIndex）？](#q87为什么不用现成的向量检索--大模型-rag-框架langchain--llamaindex)
  - [Q88：两小时视频，ASR 60 秒一片要调 120 次，不会很慢很贵吗？](#q88两小时视频asr-60-秒一片要调-120-次不会很慢很贵吗)
  - [Q89：既然有 Checkpoint，为什么还需要 MQ 重投？](#q89既然有-checkpoint为什么还需要-mq-重投)
- **[十、容量、性能与规模](#十容量性能与规模)**
  - [Q90：它能处理多大的视频？多长时间的视频？](#q90它能处理多大的视频多长时间的视频)
  - [Q91：处理能力、处理效率怎么样？](#q91处理能力处理效率怎么样)
  - [Q92：如果同时有 1000 个解析请求，系统怎么处理？](#q92如果同时有-1000-个解析请求系统怎么处理)
  - [Q93：怎么处理长短视频的资源矛盾，防止长视频饿死短视频？](#q93怎么处理长短视频的资源矛盾防止长视频饿死短视频)
  - [Q94：如果要把这个项目推到生产环境，还需要处理哪些问题？](#q94如果要把这个项目推到生产环境还需要处理哪些问题)
  - [Q95：如果面对上千用户上传长视频并反复追问，你会怎么设计缓存？](#q95如果面对上千用户上传长视频并反复追问你会怎么设计缓存)
